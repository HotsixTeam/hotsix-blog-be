...
export const requestPasswordReset = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const verifyInfo = await Verify.findOne({ where: { userId: user.id } });
    ...
    if (verifyInfo) {
      ...
      if (verifyInfo.lockUntil && verifyInfo.lockUntil > new Date()) {
        return res.status(400).json({
          ...
          error: "비밀번호 변경 시도가 잠금 상태입니다. 잠시 후 다시 시도하세요.",
        });
      }
    ...
    const mailOptions = {
      ...
      subject: "비밀번호 재설정 요청",
      text: `비밀번호 재설정을 위해 다음 코드를 입력하세요: ${resetCode}`,
    };

    await transporter.sendMail(mailOptions);

    res.status(200).json({ message: "비밀번호 재설정 코드가 이메일로 발송되었습니다." });
  } catch (error) {
    console.log(error);
...
export const verifyResetCode = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      message: "코드가 유효합니다. 이제 비밀번호를 변경할 수 있습니다.",
    });
  } catch (error) {
    console.log(error);
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!verifyInfo || !verifyInfo.isVerified) {
      await transaction.rollback();
      return res.status(400).json({ error: "비밀번호를 변경할 수 있는 상태가 아닙니다. 인증을 먼저 완료하세요." });
    }
    ...
    verifyInfo.verifyToken = null;
    verifyInfo.verifyExpiry = null;
    await verifyInfo.save({ transaction });
